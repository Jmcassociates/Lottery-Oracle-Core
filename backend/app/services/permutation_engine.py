...
class PermutationScouter:
    ...
    def is_valid_pattern(ticket: tuple, game_name: str) -> bool:
        ...
        if game_name == "Pick3":
            # Sum must be between 11 and 16 (captures the peak of the bell curve)
            if not (11 <= ticket_sum <= 16):
                return False
            # Triples (3 of a kind) only happen 1% of the time. Reject them.
            if max_repeats == 3:
                return False

        elif game_name == "Pick4":
            # Sum must be between 16 and 20
            if not (16 <= ticket_sum <= 20):
                return False
            # Quads are 0.1%, triples are 3.9%. Reject Quads and Triples.
            if max_repeats >= 3:
                return False

        elif game_name == "Pick5":
            # Sum must be between 20 and 26
            if not (20 <= ticket_sum <= 26):
                return False
            # 4 of a kind and 5 of a kind are < 0.5%. Reject them.
            if max_repeats >= 4:
                return False
...
class PermutationMathEngine:
    ...
    def is_historical_jackpot(self, ticket_tuple):
        """
        JMc - For permutations, order matters. We check the exact string sequence.
        """
    ...
    def generate_tickets(self, num_tickets):
        """
        JMc - Pick 3/4/5 are Sampling With Replacement (order matters). 
        ...
        return final_tickets, list(range(10)) # pool is always 0-9
